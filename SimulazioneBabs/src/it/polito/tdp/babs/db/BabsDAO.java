...
public class BabsDAO {      //DB molto grande!! 
	...
	public List<Trip> getAllTrips(LocalDate date) {                     //tutti i trip di una data
	...
	public int getDepartures(Station s, LocalDate date) {
		...
		String sql = "SELECT COUNT(*) as count FROM trip WHERE DATE(startdate) = ? AND startterminal= ? ";  //DATE trasforma DATETIME di sql in Date

		try {
			PreparedStatement st = conn.prepareStatement(sql);
			st.setDate(1, Date.valueOf(date));                   //Converto LocalDate in Date
	...
	public int getArrivals(Station s, LocalDate date) {
		...
		String sql = "SELECT COUNT(*) as count FROM trip WHERE DATE(enddate) = ? AND endterminal= ? ";  //DATE trasforma DATETIME di sql in Date
